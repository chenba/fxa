...
const CACHE_DURATION = 24 * 60 * 60; // a day in seconds
...
const isCacheExpired = (lastChecked) =>
  Math.round((Date.now() - lastChecked) / 1000) > CACHE_DURATION;
...
module.exports = {
  ...
  async process(req, res) {
    ...
    const languageOnlyTags = supported.filter(
      (l) => !!l.region && !checkedTags.includes(l.code)
    );
